...
flags.DEFINE_boolean(
    'adjust_temp', False,
    'Whether adjusting temperature according to augmentation cloud density')

flags.DEFINE_float(
    'adjust_temp_std', 0.2,
    'Std of temperature adjustment std.')

flags.DEFINE_string(
    'adjust_temp_params', None,
    'Detailed parameters of temperature adjustment.')

flags.DEFINE_boolean(
    'exp_to_adjust_temp', False,
    'Whether using exp info to adjust temp')

flags.DEFINE_boolean(
    'rn_avg_for_temp', False,
    'Whether using running average to adjust temp')
...
def main(argv):
  ...
  if FLAGS.dataset == 'SAYCam':
    import saycam_builder
    builder = saycam_builder.SAYCam(FLAGS.dataset, data_dir=FLAGS.data_dir)
    num_train_examples = builder.num_examples
