...
MODEL_DIR=gs://cx_visualmaster/simclr_vary_tau_20_10_30_exp

python run.py --train_mode=pretrain \
  --train_batch_size=256 --train_epochs=100 \
  --learning_rate=0.6 --weight_decay=1e-6 --temperature=0.20 \
  --dataset=imagenet2012 --image_size=224 --eval_split=validation \
  --data_dir=$DATA_DIR --model_dir=$MODEL_DIR \
  --use_tpu=True --tpu_name=$TPU_NAME --train_summary_steps=0 --tpu_zone=europe-west4-a \
  --num_transforms=4 --adjust_temp=True --adjust_temp_params=0.033,0.033,0.10,0.30 \
  --exp_to_adjust_temp=True
