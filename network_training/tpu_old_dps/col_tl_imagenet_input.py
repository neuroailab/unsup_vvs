...
class Col_Tl_Input(object):
  ...
  def __init__(self, is_training, data_dir, num_cores=8, soft=True, down_sample=8, col_knn=False, col_tl=True, combine_rp=False):
  ...
  def dataset_parser(self, value):
    ...
    labels = tf.cast(tf.reshape(parsed['labels'], shape=[]), dtype=tf.int32)

    print("data_provider output: image:", l_image)
    print("label:", labels)


    return l_image, labels
