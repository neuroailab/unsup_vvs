...
class Combine_RP_ImageNet_Input(object):
  ...
  def __init__(self, is_training, data_dir, num_cores=8, num_grids=4, g_noise=0, std=False):
    ...
    self.imn_dir = 'gs://small-imagenet'
  ...
  def dataset_parser_together(self, value):
    ...
    rp_image, _ = value['rp']
    return rp_image, imagenet_image, imagenet_label
  ...
  def input_fn(self, params):
    ...
    imn_file_pattern = os.path.join(
        self.imn_dir, 'train-*' if self.is_training else 'validation-*')

    imn_dataset = tf.data.Dataset.list_files(imn_file_pattern)
    
    if self.is_training:
      rp_dataset = rp_dataset.shuffle(buffer_size=1024)   # 1024 files in dataset
      rp_dataset = rp_dataset.repeat()
      imn_dataset = imn_dataset.shuffle(buffer_size=1024).repeat()
    ...
    imn_dataset = imn_dataset.apply(
        tf.contrib.data.parallel_interleave(
            fetch_dataset, cycle_length=self.num_cores, sloppy=True))
    imn_dataset = imn_dataset.shuffle(1024)
    ...
    dataset_parser_dict['imn'] = imn_dataset.map(
    ...
    rp_images, imagenet_images, labels = zip_dataset.make_one_shot_iterator().get_next()
    ...
    all_images['imn'] = imagenet_images
    all_images['labels'] = labels
