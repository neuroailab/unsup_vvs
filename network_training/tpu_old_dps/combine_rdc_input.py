...
class Combine_RDC_Input(object):
    ...
    def __init__(self, is_training, pbr_dir, scene_dir, num_cores=8, ab_depth=False, down_sample=1, color_dp_tl=True, rp_dp_tl=False, combine_3_task=True):
    ...
    def input_fn(self, params):
        ...
        all_images = {}
        all_images['depth_image'] = original_image
        all_images['depth_label'] = depth_image
        all_images['rp_image'] = (tf.reshape(rp_image, [batch_size * 2, 4, 96, 96, 3]))
        all_images['color_image'] = (tf.reshape(color_image, [batch_size * 2, 256, 256, 3]))
        all_images['color_label'] = (tf.reshape(color_label, [batch_size * 2, 32, 32, 313]))
        print("depth image:", all_images['depth_image'])
        print("depth label:", all_images['depth_label'])
        print("rp image:", all_images['rp_image'])
        print("color image:", all_images['color_image'])
        print("color label:", all_images['color_label'])
