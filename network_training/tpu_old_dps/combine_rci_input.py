...
class Combine_RCI_Input(object):
  ...
  def __init__(self, is_training, data_dir, num_cores=8, num_grids=4, g_noise=0, down_sample=8, std=False):
    ...
    self.small_dir = 'gs://small-imagenet'
  ...
  def dataset_parser_small(self, value):
    """Parse an ImageNet record from a serialized string Tensor."""
    keys_to_features = {
      'images': tf.FixedLenFeature((), tf.string, ''),
      'labels': tf.FixedLenFeature([], tf.int64, -1)
  ...
  def dataset_parser_together(self, value):
    ...
    rp_image, _ = value['rp']
    imn_image, imn_label = value['imn']
    return rp_image, col_image, col_label, imn_image, imn_label
  ...
  def input_fn(self, params):
    ...
    small_imn_pattern = os.path.join(
            self.small_dir, 'train-*' if self.is_training else 'validation-*')

    small_imn_dataset = tf.data.Dataset.list_files(small_imn_pattern)
    ...
    if self.is_training:
      ...
      small_imn_dataset = small_imn_dataset.shuffle(buffer_size=100).repeat()
    ...
    small_imn_dataset = small_imn_dataset.apply(
        tf.contrib.data.parallel_interleave(
            fetch_dataset, cycle_length=self.num_cores, sloppy=True))
    small_imn_dataset = small_imn_dataset.shuffle(100)
    ...
    dataset_parser_dict['col'] = col_dataset.map(
        ...
        num_parallel_calls=64)
    dataset_parser_dict['imn'] = small_imn_dataset.map(
        self.dataset_parser_small,
        num_parallel_calls=64)
    ...
    rp_images, col_images, col_labels, imn_images, imn_labels = zip_dataset.make_one_shot_iterator().get_next()
    ...
    all_images['imn_images'] = imn_images
    all_images['imn_labels'] = imn_labels
    print("rp_images:", rp_images)
    print("col_images:", col_images)
    print("col_labels:", col_labels)
    print("imn_images:", imn_images)
    print("imn_labels:", imn_labels)
