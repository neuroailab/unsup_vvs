...
class PBRSceneNetZipDepthInput(object):
    ...
    def __init__(self, is_training, pbr_dir, scene_dir, num_cores=8, ab_depth=False, down_sample=1, color_dp_tl=False, rp_dp_tl=False, rpcol_dp_tl=False):
        ...
        self.combine_3_task = rpcol_dp_tl
    ...
    def dataset_parser_together(self, value):
        ...
        return pbr_image, pbr_depth, scene_image, scene_depth
    ...
    def input_fn(self, params):
        ...
        pbr_mlt_files = tf.data.Dataset.list_files(pbr_mlt_list, seed=0)

        pbr_depth_list = self.get_tfr_filenames(pbr_depth_file_pattern)
        pbr_depth_files = tf.data.Dataset.list_files(pbr_depth_list, seed=0)

        scene_photo_list = self.get_tfr_filenames(scene_photo_file_pattern)
        scene_photo_files = tf.data.Dataset.list_files(scene_photo_list, seed=1) 

        scene_depth_list = self.get_tfr_filenames(scene_depth_file_pattern)
        scene_depth_files = tf.data.Dataset.list_files(scene_depth_list, seed=1)


        if self.is_training:
            #pbr_mlt_files = pbr_mlt_files.shuffle(buffer_size=self.pbr_training_data_num, seed=3)
            #pbr_depth_files = pbr_depth_files.shuffle(buffer_size=self.pbr_training_data_num, seed=3)
            #scene_photo_files = scene_photo_files.shuffle(buffer_size=self.scene_training_data_num, seed=2)
            #scene_depth_files = scene_depth_files.shuffle(buffer_size=self.scene_training_data_num, seed=2)
        ...
        files_dict = {'pbr_mlt': pbr_mlt_files, 'pbr_depth': pbr_depth_files, 'scene_photo': scene_photo_files, 'scene_depth': scene_depth_files}

        def fetch_dataset(filename):
            buffer_size = 128 * 1024 * 1024  # 1024 MiB per file
        ...
        pbr_image, pbr_depth, scene_image, scene_depth = zip_dataset.make_one_shot_iterator().get_next()
        ...
        if self.color_dp_tl and self.combine_3_task == False:
