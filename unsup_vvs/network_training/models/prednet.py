...
class PredNet(Recurrent):
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        if pool_layers is None:
            pool_layers = list(range(self.nb_layers))
    ...
    def get_initial_state(self, x):
        ...
        for u in states_to_pass:
            for l in range(nlayers_to_pass[u]):
                ds_factor = 2 ** np.sum(np.array(self.pool_layers) < l)
    ...
    def step(self, a, states):
        ...
        for l in reversed(range(self.nb_layers)):
            ...
            if (l > 0) and ((l-1) in self.pool_layers):
        ...
        for l in range(self.nb_layers):
            ...
            if self.output_layer_num == l:
                if self.output_layer_type == 'A':
                    ...
                    output = e[l]
            _curr_outputs['A'].append(a)
            _curr_outputs['Ahat'].append(ahat)

            if (l < self.nb_layers - 1):
                a = self.conv_layers['a'][l].call(e[l])
                if l in self.pool_layers:
                    a = self.pool.call(a)  # target for next layer
        self.all_outputs.append(_curr_outputs)
...
def get_prednet_inst(args, *prednet_args, **prednet_kwargs):
    ...
    if args.model_type == 'prednet_l9':
        stack_sizes = (n_channels, 64, 64, 64, 128, 128, 256, 256, 512, 512)
        R_stack_sizes = stack_sizes
        A_filt_sizes = tuple((3 for _ in range(9)))
        Ahat_filt_sizes = tuple((3 for _ in range(10)))
        R_filt_sizes = tuple((3 for _ in range(10)))
        pool_layers = [0, 1, 3, 5, 7, 9]
    else:
    ...
    prednet = PredNet(stack_sizes, R_stack_sizes,
                      A_filt_sizes, Ahat_filt_sizes, R_filt_sizes,
                      output_mode='error', return_sequences=True, pool_layers=pool_layers,
                      name='prednet', *prednet_args, **prednet_kwargs)
    return prednet


def build_loss(args, errors):
    # weighting for each layer in final loss; "L_0" model:  [1, 0, 0, 0], "L_all": [1, 0.1, 0.1, 0.1]
    if args.model_type == 'prednet_l9':
        layer_loss_weights = np.array([1.] + [0.] * 9)
    ...
    errors_by_time = TimeDistributed(
            ...
            trainable=False, name='time_distributed')(errors)  # calculate weighted error by layer
    errors_by_time = Flatten()(errors_by_time)  # will be (batch_size, nt)
    final_errors = Dense(
            1, weights=[time_loss_weights, np.zeros(1)], 
            trainable=False, name='dense')(errors_by_time)  # weight errors by time
    return final_errors


def get_model(args):
    # Model parameters
    prednet = get_prednet_inst(args)

    def _build_model():
        inputs = Input(shape=(nt,) + input_shape)
        errors = prednet(inputs)  # errors will be (batch_size, nt, nb_layers)
        final_errors = build_loss(args, errors)
    ...
    if num_gpus == 1:
        ...
        raise NotImplementedError('Multi-gpu training speed is much slower!')
...
def get_model_test(args):
    ...
    errors = prednet(image_placeholder)
    pdb.set_trace()
    pass
